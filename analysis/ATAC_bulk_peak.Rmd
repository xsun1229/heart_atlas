...
where $OCR_{1}$ is the our bulk peaks and $OCR_{2}$ are peaks for each cell type (not cell type specific). We sampled 2,038,760 (203,876 *10) unoverlapping regions (width = 501 which is the same with our peaks) from whole genome (hg38, blacklist and gaps were excluded) and used these regions as $\overline{OCR_{1}}$ . 


```{r echo=F, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/heart_atlas/3.ATAC_peak/random_ranges_data/OR_10times.rdata")
...
# random_bed <- read.table("random10times.bed")
# random_bed$V6 <- "."
# write.table(random_bed, file = "random10times_modify.bed", quote = F, col.names = F, row.names = F,sep = "\t")

gr_random <- readBed("random10times_modify.bed")
...
```{r echo=F, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/heart_atlas/3.ATAC_peak/random_ranges_data/OR_samesize.rdata")
DT::datatable(OR_final, options = list(pageLength =10))
```
...
```{r echo=F, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/heart_atlas/3.ATAC_peak/random_ranges_data/OR_10times_celltypespecific.rdata")
DT::datatable(OR_final, options = list(pageLength =10))
```
...
```{r echo=F, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/heart_atlas/3.ATAC_peak/random_ranges_data/OR_samesize_celltypespecific.rdata")
DT::datatable(OR_final, options = list(pageLength =10))
```